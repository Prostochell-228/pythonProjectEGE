...
def B():#сложное говно
    ...
    for q,w,e in g:#Интересный перебор который распокоувает из скобок смотреть G
        ...
        b = [0 for i in range(k)]#Создаём список который хранит максимально возможные перемножения второго и третьего чисел
        while a3[len(b)+k:]:# Заполняет БЭ смотреть выше
            m = max(a3[len(b)+k:])#Находи максимально возможные значения из нерассмотренных значений A3 смотреть вышле
        ...
        while b[len(c)+k:]:#Заполнет ЦЭ смотреть выше
            m = max(b[len(c)+k:])#Находи максимально возможные значения из нерассмотренных значений БЭ смотреть вышле
    ...
    print(maxi)#ТЫ ЕБЛАН ТУТ ЧИТАТЬ ЭТО ПРИНТ БЛЯТЬ
